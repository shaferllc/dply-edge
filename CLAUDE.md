...
>   `/projects/{server}/sites/{site}/…` shape they were built on (`/servers/…` redirects there). The model is a
>   thin record — every SSH/provisioning method left with the VM platform.
> - **`/dashboard` is the product list** (route `dashboard`). `/projects` 301s there. One product surface, so the
